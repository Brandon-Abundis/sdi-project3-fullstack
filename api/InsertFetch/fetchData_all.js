...
async function fetchCountries() {
  ...
  const restCountries = await fetch("https://restcountries.com/v3.1/all?fields=name,cca2,capital,region,coatOfArms,population,area,flags,maps,subregion"
  ).then(res => res.json());
  // filter out countries with missing, flags, and coat of arms...
  const filteredCountries = restCountries
    ...
    // literally some countries are missing a flag and coat for some reason.
    .map(country => ({
  ...
  fs.writeFile("backup_no_filter.json", countryString, err => {
